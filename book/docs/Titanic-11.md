...
캐글이 2012년에 처음 공개한 대회가 바로 이 타이타닉 생존자 예측 대회였다는 사실을 아시나요? 데이터 사이언스 입문자들을 위해 특별히 선정된 이 데이터셋에는 흥미로운 비하인드 스토리가 있습니다. 타이타닉 승객 데이터는 영국 선박부에서 작성한 공식 기록을 바탕으로 합니다. 하지만 당시의 모든 정보가 완벽하게 보존된 것은 아니었습니다. 특히 3등석 승객들의 정보는 상당 부분 누락되어 있었고, 생존자 명단도 여러 번의 수정을 거쳤습니다. 캐글은 이 불완전한 데이터를 교육용으로 가공하면서, 일부러 Age나 Cabin 같은 중요한 정보들의 결측치를 그대로 남겨두었습니다. 이는 현실 세계의 데이터도 완벽하지 않다는 것을 보여주기 위해서였죠. 실제로 많은 데이터 사이언티스트들이 이 대회를 통해 결측치 처리, 범주형 변수 인코딩 등 데이터 전처리의 기초를 배웠다고 합니다. 그래서 타이타닉 대회는 "데이터 사이언스의 Hello World"라고도 불립니다. 
